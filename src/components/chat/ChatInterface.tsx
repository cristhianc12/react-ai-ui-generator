...
export function ChatInterface() {
  ...
  if (messages.length === 0) {
    return (
      <div className="flex flex-col h-full p-4 overflow-hidden">
        ...
        {error && (
          <div className="mb-3 flex items-start gap-2 px-3 py-2.5 rounded-lg bg-red-50 border border-red-200 text-sm text-red-700">
  ...
  return (
    <div className="flex flex-col h-full p-4 overflow-hidden">
      ...
      {error && (
        <div className="mt-3 flex items-start gap-2 px-3 py-2.5 rounded-lg bg-red-50 border border-red-200 text-sm text-red-700 flex-shrink-0">
          <AlertCircle className="h-4 w-4 mt-0.5 shrink-0" />
          <span>{error.message || "Something went wrong. Please try again."}</span>
        </div>
      )}
